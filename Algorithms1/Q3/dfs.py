# Time Complexity: O(N), where N is the number of vertexes because we only need to detect the first cycle.
...
from Algorithms1.Q3.graph import Graph


def DFS_VIST(node, color, cycle):
    ...
    for child in node.next:
        ...
        if color[child] == "WHITE":
            visited = DFS_VIST(node, color, cycle)
            if not visited:
...
def find_cycles(graph: Graph):
    ...
    color = ["WHITE"] * len(graph.v)
    cycles = []

    for i in range(graph.v):
        if color[i] == "WHITE":
            found_cycle = DFS_VIST(i, color, [])
            if found_cycle:
